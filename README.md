...
1. **The Bazaar extension's placement is empirically confirmed, its internal shape is still a best-effort guess.** Coinbase hasn't published a formal schema for this extension. An earlier version of this package nested it under `accepts[0].extra.bazaar` — Coinbase's live validator rejected a real deployed route with every other v2 check passing and named the exact problem: `"No bazaar extension in top-level extensions object"`. It now lives at `{ ...envelope, extensions: { bazaar: {...} } }`, confirmed by that same validator subsequently evaluating (not skipping) every `bazaar.*` check. The fields *inside* `bazaar` (`info.input`, `info.output`, `schema`) are still inferred from validator check names, not a published spec — run `validateDiscoveryExtension()` against your real route before trusting the internal shape is exactly right.
